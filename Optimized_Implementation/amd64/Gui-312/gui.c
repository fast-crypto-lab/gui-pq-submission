...
{
	uint8_t _z[_SEC_M_BYTE] = {0};
	//uint8_t _y[_SEC_M_BYTE] = {0};
	uint8_t x[_PUB_N_BYTE] ;

	uint8_t temp_t[_PUB_N_BYTE]  = {0};
	uint8_t Y[_SEC_M_BYTE]  = {0};
	uint8_t vinegar[(_VINEGAR_BYTE)]  = {0};
...
{
	uint8_t _w[_SEC_M_BYTE] = {0};
	uint8_t X[_PUB_N_BYTE] ;
	uint8_t y[_PUB_N_BYTE]  = {0};
...
void gui_central_map( uint8_t * y , const gui_key * sk , const uint8_t * x ) {
	...
	uint8_t c_poly[BGF_SIZE*C_TERMS] ;
...
unsigned gui_ivs_central_map( uint8_t * x , const gui_key * sk , const uint8_t * y , const uint8_t * vinegar ) {
	...
	uint8_t sp_c_poly[BGF_SIZE*C_TERMS] ;
